...
class Detector():
  def __init__(self,
               model_path=os.path.join(model_folder, 'yolov3.h5')):
    assert os.path.exists(model_path), f'Missing object_detection module model. (see /model)'
  ...
  def do_detect(self, image_path, save_directory):
    draw_boxes(image_path, None, save_directory)
    return
...
if __name__ == "__main__":
  pass
  D = Detector()
  D.do_detect(image_path='./dt/1161096.png')
  for fn in os.listdir('dt'):
      print(f'doing {fn}')
      D.do_detect(image_path=f'./dt/{fn}')
      input('continue?')
