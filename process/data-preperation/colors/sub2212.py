...
def get_image_dominant_color(file_path) -> tuple[str, str]:
  ...
  for i, d in enumerate(dominant_color):
    closest_name = get_closest_color_name(d)
    if i == 0 and closest_name.startswith('grey'):
      continue
...
if typing.TYPE_CHECKING:
  # just importing loads tensorflow things
  from object_detection.detection import Detector as ObjectDetector

detector: typing.Optional['ObjectDetector'] = None  # will be loaded later


def crop_image(file_path, dest):
  global detector
  if not detector:
    print('Loading detector for the first time...')
    detector = ObjectDetector(model_path='object_detection/model/yolov3.h5')

  try:
    return detector.do_detect(file_path, dest)
  except Exception as e:
...
if __name__ == '__main__':
  ...
  from object_detection import Detector as ObjectDetector
